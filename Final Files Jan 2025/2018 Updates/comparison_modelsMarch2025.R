...
data <- read_csv("combined_datasetMarch2025.csv")
...
# diagnostics for convergences and p values
...
run_model1 <- function(prepared_data) {
  ...
  model1 <- MCMCglmm(model1_formula,
                     ...
                     pr = TRUE,
                     verbose = TRUE)
  #convergence diagnostics and Bayesian P-value
  convergence <- monitor_convergence(model1)
  pvalue <- calc_bayesian_pvalue(model1, prepared_data$bcnh_growthindex)
  
  model1$convergence <- convergence
  model1$bayesian_pvalue <- pvalue
...
run_model2 <- function(prepared_data) {
  ...
  convergence <- monitor_convergence(model2)
  pvalue <- calc_bayesian_pvalue(model2, prepared_data$bcnh_growthindex)
  
  model2$convergence <- convergence
  model2$bayesian_pvalue <- pvalue
